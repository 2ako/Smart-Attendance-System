...
export default function AdminMakeUpClassesPage() {
    ...
    const handleReject = async (requestId: string) => {
        if (!confirm(t("confirm_reject") || "Are you sure you want to reject this request?")) return;
    ...
    const filteredRequests = requests.filter((req) => {
        const matchesSearch =
            req.professor?.name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
            req.subject?.name?.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            case "approved":
                return <Badge className="bg-emerald-500/10 text-emerald-500 border-border/50 gap-1.5"><CheckCircle2 size={12} /> {t("approved_status")}</Badge>;
            case "rejected":
                return <Badge className="bg-destructive/10 text-destructive border-border/50 gap-1.5"><XCircle size={12} /> {t("rejected_status")}</Badge>;
            default:
                return <Badge className="bg-amber-500/10 text-amber-500 border-border/50 gap-1.5"><Clock size={12} /> {t("pending_status")}</Badge>;
        }
    };
                        ...
                        <Table>
                            <TableHeader className="bg-muted/30 border-b border-border/50">
                                <TableRow className="hover:bg-transparent border-none">
                                    <TableHead className="py-6 px-8 text-[10px] font-black uppercase tracking-widest text-muted-foreground">{t("professor")}</TableHead>
                                    <TableHead className="py-6 px-8 text-[10px] font-black uppercase tracking-widest text-muted-foreground">{t("makeup_subject")}</TableHead>
                                    <TableHead className="py-6 px-8 text-[10px] font-black uppercase tracking-widest text-muted-foreground">{t("makeup_date")}</TableHead>
                                    <TableHead className="py-6 px-8 text-[10px] font-black uppercase tracking-widest text-muted-foreground">{t("status")}</TableHead>
                                    <TableHead className="py-6 px-8 text-[10px] font-black uppercase tracking-widest text-muted-foreground ltr:text-right rtl:text-left">{t("actions")}</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {isLoading ? (
                                        ...
                                        <TableRow key={item._id} className="hover:bg-muted/20 border-border/50 transition-colors group">
                                            <TableCell className="py-6 px-8">
                                                <div className="flex items-center gap-4 text-start">
                                                    <div className="h-12 w-12 rounded-2xl bg-primary/10 flex items-center justify-center text-primary font-bold transition-transform group-hover:scale-110">
                                                        <User size={20} />
                                                    </div>
                                                    <div>
                                                        <p className="font-black text-foreground group-hover:text-primary transition-colors text-start">{item.professor?.name}</p>
                                                        <p className="text-[10px] font-bold text-muted-foreground uppercase text-start">{item.professor?.specialization || t("professor")}</p>
                                                    </div>
                                                </div>
                                            </TableCell>
                                            <TableCell className="py-6 px-8">
                                                <div className="max-w-[250px] text-start">
                                                    ...
                                                    <Badge variant="outline" className="text-[10px] mt-1 border-primary/20 bg-primary/5 text-primary text-start uppercase font-black tracking-tighter">
                                            ...
                                            <TableCell className="py-6 px-8 ltr:text-right rtl:text-left">
                                                <div className="flex items-center ltr:justify-end rtl:justify-start gap-2 text-start">
                                                    ...
                                                    {item.status === "approved" && (item.roomName || item.room) && (
                                                        <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-emerald-500/5 border border-emerald-500/10 text-[10px] font-bold text-emerald-600 uppercase text-start">
                                                            <MapPin size={12} />
                                                            {item.roomName || (item.room?.name || item.room)}
                                                        </div>
                                                    )}
                                                </div>
                                            </TableCell>
